((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_36",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,E,F,B={
yG(d){var x=0,w=A.j(y.n),v,u,t,s,r,q,p
var $async$yG=A.e(function(e,f){if(e===1)return A.f(f,w)
while(true)switch(x){case 0:t=y.g
s=A.ad(["isall",""+d],t,t)
q=C.t
p=C.w
x=3
return A.c(G.dS(F.ch("/getBookSources"),s),$async$yG)
case 3:r=q.an(0,p.an(0,f))
t=J.a1(r)
if(t.h(r,"isSuccess")){t=J.fB(t.h(r,"data"),new B.alL(),y.l)
u=A.a9(t,!0,t.$ti.i("aQ.E"))
if(d===0)$.yw=u
v=u
x=1
break}else throw A.d(A.aX(E.cz(t.h(r,"errorMsg"))))
case 1:return A.h(v,w)}})
return A.i($async$yG,w)},
alL:function alL(){},
be0(d){var x,w,v,u,t,s=null,r=J.a1(d),q=r.h(d,"bookSourceUrl")
q=q==null?s:J.al(q)
if(q==null)q=""
x=r.h(d,"bookSourceName")
x=x==null?s:J.al(x)
if(x==null)x=""
w=D.ajf(r.h(d,"enabledExplore"))
v=D.ajf(r.h(d,"enabled"))
u=r.h(d,"bookSourceGroup")
u=u==null?s:J.al(u)
if(u==null)u=""
t=r.h(d,"loginUrl")
t=t==null?s:J.al(t)
if(t==null)t=""
r=r.h(d,"loginUi")
r=r==null?s:J.al(r)
return new B.ef(q,x,w,v,u,t,r==null?"":r)},
ef:function ef(d,e,f,g,h,i,j){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j}},G,D
J=c[1]
A=c[0]
C=c[2]
E=c[124]
F=c[115]
B=a.updateHolder(c[71],B)
G=c[122]
D=c[76]
B.ef.prototype={}
var z=a.updateTypes(["ef(@)"])
B.alL.prototype={
$1(d){return B.be0(d)},
$S:z+0};(function inheritance(){var x=a.inherit
x(B.alL,A.bC)
x(B.ef,A.v)})()
var y={n:A.w("D<ef>"),l:A.w("ef"),g:A.w("l")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_36",e:"endPart",h:b})})($__dart_deferred_initializers__,"v60W8/FoSqCzB6eNFW/ZQ85jAhY=");