((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_87",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,D,E,A={
bG5(d,e,f){var x,w,v,u,t,s,r=e.b
if(r<=0||e.a<=0||f.b<=0||f.a<=0)return C.Qz
switch(d.a){case 0:x=f
w=e
break
case 1:v=f.a
u=f.b
t=e.a
x=v/u>t/r?new B.I(t*u/r,u):new B.I(v,r*v/t)
w=e
break
case 2:v=f.a
u=f.b
t=e.a
w=v/u>t/r?new B.I(t,t*u/v):new B.I(r*v/u,r)
x=f
break
case 3:v=f.a
u=f.b
t=e.a
if(v/u>t/r){w=new B.I(t,t*u/v)
x=f}else{x=new B.I(v,r*v/t)
w=e}break
case 4:v=f.a
u=f.b
t=e.a
if(v/u>t/r){x=new B.I(t*u/r,u)
w=e}else{w=new B.I(r*v/u,r)
x=f}break
case 5:w=new B.I(Math.min(e.a,f.a),Math.min(r,f.b))
x=w
break
case 6:s=e.a/r
v=f.b
x=r>v?new B.I(v*s,v):e
r=f.a
if(x.a>r)x=new B.I(r,r/s)
w=e
break
default:w=null
x=null}return new A.WW(w,x)},
uZ:function uZ(d,e){this.a=d
this.b=e},
WW:function WW(d,e){this.a=d
this.b=e},
bIg(d,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,b0,b1,b2,b3,b4){var x,w,v,u,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f,e
if(b2.gap(0))return
x=b2.a
w=b2.c-x
v=b2.b
u=b2.d-v
t=new B.I(w,u)
s=a8.gf8(a8)
r=a8.gbp(a8)
if(a6==null)a6=C.Lc
q=A.bG5(a6,new B.I(s,r).iL(0,b4),t)
p=q.a.av(0,b4)
o=q.b
if(b3!==C.h5&&o.k(0,t))b3=C.h5
n=$.aj().aA()
n.sT8(!1)
if(a3!=null)n.saH3(a3)
n.sak(0,E.iN(0,0,0,B.M(b1,0,1)))
n.st4(a5)
n.sT6(a9)
n.smq(a0)
m=o.a
l=(w-m)/2
k=o.b
j=(u-k)/2
u=d.a
u=x+(l+(a7?-u:u)*l)
v+=j+d.b*j
i=new B.H(u,v,u+m,v+k)
h=b3!==C.h5||a7
if(h)a1.fa(0)
v=b3===C.h5
if(!v)a1.rw(b2)
if(a7){g=-(x+w/2)
a1.co(0,-g,0)
a1.qn(0,-1,1)
a1.co(0,g,0)}f=d.aMa(p,new B.H(0,0,s,r))
if(v)a1.w5(a8,f,i,n)
else for(x=A.bEQ(b2,i,b3),w=x.length,e=0;e<x.length;x.length===w||(0,B.X)(x),++e)a1.w5(a8,f,x[e],n)
if(h)a1.e1(0)},
bEQ(d,e,f){var x,w,v,u,t,s,r=e.c,q=e.a,p=r-q,o=e.d,n=e.b,m=o-n,l=f!==C.S1
if(!l||f===C.S2){x=D.c.ev((d.a-q)/p)
w=D.c.iY((d.c-r)/p)}else{x=0
w=0}if(!l||f===C.S3){v=D.c.ev((d.b-n)/m)
u=D.c.iY((d.d-o)/m)}else{v=0
u=0}r=B.a([],y.b)
for(t=x;t<=w;++t)for(q=t*p,s=v;s<=u;++s)r.push(e.dW(new B.p(q,s*m)))
return r},
zQ:function zQ(d,e){this.a=d
this.b=e}},C
B=c[0]
D=c[2]
E=c[121]
A=a.updateHolder(c[88],A)
C=c[168]
A.uZ.prototype={
I(){return"BoxFit."+this.b}}
A.WW.prototype={}
A.zQ.prototype={
I(){return"ImageRepeat."+this.b}}
var z=a.updateTypes([]);(function inheritance(){var x=a.inheritMany,w=a.inherit
x(B.e5,[A.uZ,A.zQ])
w(A.WW,B.v)})()
var y={b:B.w("u<H>")};(function constants(){C.pp=new A.uZ(1,"contain")
C.dj=new A.uZ(2,"cover")
C.fG=new A.uZ(3,"fitWidth")
C.Lb=new A.uZ(4,"fitHeight")
C.Lc=new A.uZ(6,"scaleDown")
C.Qz=new A.WW(D.F,D.F)
C.S1=new A.zQ(0,"repeat")
C.S2=new A.zQ(1,"repeatX")
C.S3=new A.zQ(2,"repeatY")
C.h5=new A.zQ(3,"noRepeat")})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_87",e:"endPart",h:b})})($__dart_deferred_initializers__,"W2y4tdiWfTBCMMMUC1a0dQy1Qxk=");